...
export function updateBoard(board) {
    return function (dispatch) {
        try {
            boardApi.updateBoard(board).then(res => dispatch(updateBoardSuccess(res.data)));
        } catch (error) {
            return dispatch(loadFoldersFailed(error.message));
        }
    };
...
export function deleteBoard(board) {
    return function (dispatch) {
        try {
            boardApi.deleteBoardById(board._id).then(res => dispatch(deleteBoardSuccess(res.data)));
        } catch (error) {
            return dispatch(loadFoldersFailed(error.message));
        }
    };
