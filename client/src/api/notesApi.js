...
const api = {
  ...
  saveNote: (note) => {
    ...
    return axios.post('/api/notes', { data: note, headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}`, 'Access-Control-Allow-Methods': '*' }, mode: 'cors' }).then(res => res.data);
  },

  updateNote: (note) => {
    axios.defaults.headers.common.Authorization = `Bearer ${token}`;

    return axios.put('/api/notes', { method: 'PUT', data: note, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Methods': '*' } }).then(res => res.data);
  },
